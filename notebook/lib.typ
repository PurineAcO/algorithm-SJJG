...
#let config = (
  ...
  title1-size: font-size.n3,
  title2-size: font-size.s3,
  title3-size: font-size.n4,
  titlel-size: font-size.s4,
  // 字体设置
...
#let explain(it) = {
  ...
  let body = [*Ans.*] + h(config.block-space) + it
  box(stroke: config.table-stroke,
  block(
    inset: 8pt,
  ...
  align(center)[
    ...
    #block(
      text(
        ...
        // weight: "bold",
        16pt,
        [_Last Update_: #datetime.today().display("[year].[month].[day]")],
      )
    )
    #v(1.1fr)
  ]
...
#let project(
  ...
  set enum(
    ...
    numbering: (..n) => {
      n = n.pos()
      let level = n.len()
      let number = config.enum-numbering.at(level -1, default: "a.")
  ...
  show heading: it => {
    ...
    let body = if it.numbering != none {counter(heading).display() + h(config.small-space) + it.body} 
    else {
      it.body
    }
    box(width: 100%, body)
  ...
  show math.equation: it => {
    ...
    if it.block and not it.has("label") [
      ...
      #math.equation(it.body, block: true, numbering: none)#label("(1)")
    ] else {
      it
    }
  }
